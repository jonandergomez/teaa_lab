...
if __name__ == "__main__":
    ...
    # -------------------------------------------------------------------------------------------------------
    def kernel(pair, bw):
        a = pair[0] # tuple: id, 21x20 array
        x = pair[1] # should be a 21x20 array
        xa = a[1] # 21x20 array
        two_norm = (((xa - x) / bw) ** 2).sum(axis = 1) # 21-dimensional array
        similarity = numpy.exp(-0.5 * two_norm).sum() # one float
    ...
    for subset, rdd_data in zip(['train', 'test'], [rdd_train_data, rdd_test_data]):
        ...
        temp = rdd_data.zipWithIndex()
        y_true   = temp.map(lambda t: (t[1], t[0][0]))
        data_with_ids = temp.map(lambda t: (t[1], t[0][1]))
        print(subset, y_true.count(), y_true.getNumPartitions())
        print(subset, data_with_ids.count(), data_with_ids.getNumPartitions())
        similarities = None
        for label in numpy.unique(label_mapping):
            last_time = time.time()
            d = rdd_train_data.filter(lambda t: t[0] == label).map(lambda x: x[1])
            d_count = d.count()
            if d_count > 0:
                _ = data_with_ids.cartesian(d).map(lambda pair: kernel(pair, band_width)).reduceByKey(lambda a, b: a + b, num_partitions)
                if _.count() > 0:
                    _ = _.map(lambda t: (t[0], t[1] / d_count))
                    if similarities is None:
                        similarities = _.map(lambda t: (t[0], [t[1]]))
                    else:
                        similarities = similarities.join(_).map(lambda t: (t[0], t[1][0] + [t[1][1]]), num_partitions)
            #
            print(subset, 'prepared the computation of densities per label', label, 'in', time.time() - last_time, 'seconds')
            last_time = time.time()
        #
        last_time = time.time()
        y_pred = similarities.map(lambda t: (t[0], numpy.array(t[1]).argmax()))
        y_true_pred = y_true.join(y_pred).map(lambda t: t[1]).collect()
        print('computed predictions in', time.time() - last_time, 'seconds')
