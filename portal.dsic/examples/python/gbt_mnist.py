...
def main(args, sc):
    ...
    for nt in args.numTrees.split(sep = ':'):
        ...
        for md in args.maxDepth.split(sep = ':'):
            ...
            gbtr = GBTRegressor(labelCol="label", featuresCol="features", maxIter=numTrees, maxDepth=maxDepth, impurity=args.impurity)

            # Creating the pipeline, in this case with only one component, the model based on RandomForest 
            pipeline = Pipeline(stages=[preprocessLabels, gbtr, postprocessLabelsAndPredictions])
