...
grep -H "^   macro avg" results.digits.train/kmeans-classification-results-*.txt | cut -f4 -d'-' | sed 's/\.txt://g' | awk '{ print $1, $3 }' | sort -nk 1 >/tmp/temp.train.$$
grep -H "^   macro avg" results.digits.test/kmeans-classification-results-*.txt  | cut -f4 -d'-' | sed 's/\.txt://g' | awk '{ print $1, $3 }' | sort -nk 1 >/tmp/temp.test.$$
