...
2. Compute the confusion matrix between target classes and a given number of clusters so
   ...
   $$P(\mathcal{C}_k \mid x)$$, the *a posteriori* probability that the vector
...
5. Compute a confusion matrix between target classes and the components of the GMM for different number
   ...
   conditional probabilities $P(w_j \mid \mathcal{C}_k)$, where $w_j$ is the $j$-th component of 
   the GMM and $\mathcal{C}_k$ is the $k$-th target class.
   ...
   $$P(\mathcal{C}_k \mid x) = \frac{\pi(\mathcal{C}_k) p(x \ mid \mathcal{C}_k)}{\sum_c \pi(\mathcal{C}_c) p(x \ mid \mathcal{C}_c)}$$

   where
   $\pi(\mathcal{C}_k)$ is the *a priori* probability of the target class $\mathcal{C}_k$,
   $p(x \mid \mathcal{C}_k)$ is the *conditional probability density* of generating $x$ during the state corresponding to target class $\mathcal{C}_k$.

   The *conditional probability density* is computed as follows:

   $$p(x \mid \mathcal{C}_k) = \sum_{j=1}^J \pi(w_j) p(x \mid w_j) P(w_j \mid \mathcal{C}_k)$$,

   where
   $J$ is the number of components in the GMM used,
   $\pi(w_j)$ is the weight of Gaussian $w_j$ in the mixture,
   $p(x \mid w_j) = \mathcal{N}(x \mid \mu_{j}, \Sigma_{j})
   and $P(w_j \mid \mathcal{C}_k)$ is the conditional probability of observing
